...
void wifi_connexion()
{
  ...
  while (WiFi.status() != WL_CONNECTED) { //Tant que la connexion au wifi ne se fait pas
    light_led(1, 100); //Indication de connexion au WIFI via LED Rouge
  }
}
...
void cardPresent()
{
  if (rfid.PICC_IsNewCardPresent())  // on a détecté un tag
  {
    if (rfid.PICC_ReadCardSerial())  // on a lu avec succès son contenu
    {
      ...
      for (byte i = 0; i < uid.length(); i++) //Transforme les lettres minuscule en lettre majuscule
      {
        if (uid[i] >= 'a' && uid[i] <= 'z')
        {
          uid[i] = uid[i] - 32;
      ...
      unlock_request(uid); //Fait la requête d'ouverture vers l'API
      delay(1000);
    }
  }
}
